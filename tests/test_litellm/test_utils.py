...
@pytest.mark.parametrize(
    ...
    [
        ...
        (LlmProviders.DEEPINFRA, "DeepInfraConfig"),
...
def test_get_provider_model_info_resolves_every_supported_provider(provider, expected_class_name):
    from litellm.utils import ProviderConfigManager

    config = ProviderConfigManager.get_provider_model_info(model="some-model", provider=provider)

    assert type(config).__name__ == expected_class_name


def test_get_provider_model_info_returns_none_for_provider_without_model_info():
    from litellm.utils import ProviderConfigManager

    assert ProviderConfigManager.get_provider_model_info(model="c", provider=LlmProviders.COHERE) is None
